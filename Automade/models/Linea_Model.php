<?php class Linea_Model extends CI_Model {
  	...
  	public function processUpdate($postData,$id){
     	$this->db->where("id=",$id);
    	$this->db->update('linea',$postData);
    	return $this->db->affected_rows();
  	}
  
  	public function processDelete($id){
   		$this->db->query("Update linea set is_deleted='true' where id=".$id);
    	return $this->db->affected_rows();
  	}
  	...
  	public function getByID($id){
  	  	$query = $this->db->get_where('linea', array('id' => $id,'is_deleted'=>'false' ));
    	return $query->row();
  	}  
  
  	public function getSelectData(){
    	$this->db->where("is_deleted =",'false');
        $haveName=name_exist_for_table($this,'linea');
        if($haveName==true){
	...
	public	function total_paginados($por_pagina,$segmento){
            //$consulta = $this->db->get('linea',$por_pagina,$segmento);
            $this->db->select('*');
            $this->db->from('linea');
            $this->db->where(array('is_deleted'=>'false' ));
            if(count($this->array_or_where)>0){            
