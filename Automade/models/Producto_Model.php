<?php class Producto_Model extends CI_Model {
  	...
  	public function processNew($postData){
    	$this->db->insert('producto',$postData);
  	...
  	public function processUpdate($postData,$id){
     	$this->db->where("id=",$id);
    	$this->db->update('producto',$postData);
    	return $this->db->affected_rows();
  	}
  
  	public function processDelete($id){
   		$this->db->query("Update producto set is_deleted='true' where id=".$id);
    	return $this->db->affected_rows();
  	}
  	...
  	public function getByID($id){
  	  	$query = $this->db->get_where('producto', array('id' => $id,'is_deleted'=>'false' ));
    	return $query->row();
  	}  
  
  	public function getSelectData(){
    	$this->db->where("is_deleted =",'false');
        $haveName=name_exist_for_table($this,'producto');
        if($haveName==true){
