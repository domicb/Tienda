<?php class Usuario_Model extends CI_Model {
  	...
  	public function processUpdate($postData,$id){
     	$this->db->where("id=",$id);
    	$this->db->update('usuario',$postData);
    	return $this->db->affected_rows();
  	}
  
  	public function processDelete($id){
   		$this->db->query("Update usuario set is_deleted='true' where id=".$id);
    	return $this->db->affected_rows();
  	}
  	...
  	public function getByID($id){
  	  	$query = $this->db->get_where('usuario', array('id' => $id,'is_deleted'=>'false' ));
    	return $query->row();
  	}  
  
  	public function getSelectData(){
    	$this->db->where("is_deleted =",'false');
        $haveName=name_exist_for_table($this,'usuario');
        if($haveName==true){
